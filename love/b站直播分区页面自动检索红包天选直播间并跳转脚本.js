...
window.addEventListener('load', function () {
	...
	let max_num = 5; // 最大打开直播间数量
	let page = 1; // 请求传入页面的参数
	let key_flag = 0;
	let roomid_list = []; // roomid数组
	let interval_alert_div = null; // 弹窗定时器
	let page_type = 1; // 打开页面类型 0红包+天选 1红包 2天选
	let loop_run_interval = null; // 自动循环运行定时器
	let loop_run_time = 15; // 自动循环时间间隔 毫秒,默认15分钟
	let auto_close_interval = 3; // 循环判断直播间有无红包时间间隔 默认3分钟
	let auto_close = false; // 是否启用无红包自动关闭直播间功能 false是禁用 true是启用
	let auto_remove = false; // 是否启用自动删除无用内容 false是禁用 true是启用
